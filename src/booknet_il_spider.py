...
class BooknetIlSpider(BaseSpider):
    """
    Tzomet Sfarim / Booknet (booknet.co.il) — Israel main-universe bookstore.

    Israeli bookstore chain with Hebrew and English titles; strong English-
    language section.  Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse paths probed at startup.  Pagination: ?page=N.
    """

    BASE_URL = "https://www.booknet.co.il"

    BROWSE_CANDIDATES = [
        "/קטגוריות/english-books",
        "/קטגוריות",
        "/books",
        "/english-books",
        "/shop",
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Tzomet Sfarim", territory="Israel")
    ...
    def run(self):
        self.logger.info(
            f"Starting Tzomet Sfarim harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tzomet Sfarim / Booknet Israel cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    BooknetIlSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
