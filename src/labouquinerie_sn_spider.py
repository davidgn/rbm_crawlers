...
class LaBouquinerieSNSpider(BaseSpider):
    """
    La Bouquinerie (labouquinerie.sn) — Senegal main-universe used-book marketplace.

    Senegalese bouquinerie (secondhand book dealer) with an online catalog of
    French and African-interest titles.  Captures ISBNs, pricing, condition,
    ...
    BASE_URL = "https://www.labouquinerie.sn"
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="La Bouquinerie", territory="Senegal")
    ...
    def run(self):
        self.logger.info(
            f"Starting La Bouquinerie harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="La Bouquinerie Senegal cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    LaBouquinerieSNSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
