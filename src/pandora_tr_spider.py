...
class PandoraTrSpider(PlaywrightSearchSpider):
    """
    Broad crawler for Pandora (Turkey).
    """
    def __init__(self, search_term: str = "kitap", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Pandora",
            territory="Turkey",
            base_url="https://www.pandora.com.tr",
            search_path=f"Arama/?type=9&kelime={search_term}&page={{page}}",
            currency="TRY",
            limit_pages=limit_pages,
            limit_items=limit_items,
            item_pattern=r'(<div[^>]*class="[^"]*product-item[^"]*"[^>]*>.*?</div>)',
            url_regex=r'href="([^"]+)"',
            price_regex=r'class="price">([\d,]+(?:\.\d{2})?)',
            title_regex=r'class="product-title"[^>]*>([^<]+)<',
