...
class EsliteTwSpider(BaseSpider):
    ...
    def _search(self, query: str):
        response = self.client.get(self.SEARCH_URL, params={"q": query})
    ...
    def _price(self, fields: dict) -> str | None:
        raw = fields.get("final_price")
        if not raw:
            return None
        amount = re.sub(r"[^\d.]", "", raw)
        return f"TWD {amount}" if amount else None
...
if __name__ == "__main__":
    ...
    EsliteTwSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
