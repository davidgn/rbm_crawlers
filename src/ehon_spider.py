...
class EhonSpider(BaseSpider):
    """
    e-hon (www.e-hon.ne.jp) — Japan main-universe bookstore.

    ToHan-operated nationwide online bookstore with pickup through ~3,000
    affiliated stores.  Broad new-book catalog; a top-tier platform rail.
    Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse paths probed at startup.  Pagination: pageNo=N query parameter.
    """

    BASE_URL = "https://www.e-hon.ne.jp"

    BROWSE_CANDIDATES = [
        "/bec/SA/List?johoId=050&cnt=40&dispType=listType",
        "/bec/SA/List?johoId=050",
        "/books",
        "/search",
        "/catalog",
        "",
    ]
    DETAIL_SIGNALS = ["/bec/SA/Detail", "/SA/Detail", "/book/", "/detail/", "/product/"]
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="e-hon", territory="Japan")
    ...
    def run(self):
        self.logger.info(
            f"Starting e-hon harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
        ...
        try:
            ...
            for pg_num in range(1, self.limit_pages + 1):
                ...
                if pg_num == 1:
                    ...
                    urls_to_try = [
                        f"{browse_url}{sep}pageNo={pg_num}",
    ...
    def _harvest_item(self, url: str):
        m = re.search(r"[?&]isbn=([^&]+)", url) or re.search(r"[?&]List13=([^&]+)", url)
        if m:
            item_id = re.sub(r"[^a-zA-Z0-9_-]", "_", m.group(1))[:80]
        else:
            slug = next(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="e-hon Japan cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    EhonSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
