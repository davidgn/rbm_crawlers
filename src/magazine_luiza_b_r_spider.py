...
class MagazineLuizaBRSpider(NodriverSearchSpider):
    """Broad crawler for Magazine Luiza (BR) (Brazil)."""
    def __init__(self, search_term: str = "libros", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Magazine Luiza (BR)",
            territory="Brazil",
            base_url="https://www.magazineluiza.com.br",
            search_path="busca/{search_term}/",
            currency="BRL",
            limit_pages=limit_pages,
            limit_items=limit_items,
            item_pattern=r'(<div[^>]*class="[^"]*(?:product|item|card)[^"]*"[^>]*>.*?</div>)',
            url_regex=r'href="([^"]+)"',
            price_regex=r'(?:\$|BRL|)[\s]*([\d,]+(?:\.\d{2})?)',
