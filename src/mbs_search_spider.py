...
class MbsSearchSpider(BaseSpider):
    """Base search spider for MBS Systems independent campus bookstore platforms."""
    ...
    def _get_search_url(self, search_term: str, page: int = 1) -> str:
        ...
        return f"{self.base_url}/search?q={encoded_term}&page={page}"

    def run(self, search_term: str = "potter") -> list[BookListing]:
        self.logger.info(f"Starting MBS Systems search crawler for {self.platform_name}. Limit: {self.limit_pages} pages.")
        scraped_items = []
        ...
        for page in range(1, self.limit_pages + 1):
            ...
            self.logger.info(f"Fetching MBS page {page}: {url}")
            try:
                ...
                items = soup.select('.product-item, .search-result, .item-card, tr.product-row')
                if not items:
                    # Generic fallback
                    items = soup.select('div.product, div.book-item, li.product')
                ...
                page_count = 0
                for item in items:
                    title_elem = item.select_one('.title, .product-title, .name, h2, h3, a.product-link')
                    price_elem = item.select_one('.price, .product-price, .amount, .regular-price')
                ...
                self.logger.error(f"Error scraping MBS page {page}: {e}")
                break
