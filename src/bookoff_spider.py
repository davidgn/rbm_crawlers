...
class BookoffSpider(BaseSpider):
    """
    BOOKOFF公式オンラインストア (shopping.bookoff.co.jp) — Japan main-universe.

    Japan's largest used-book/media recommerce rail; 400万点+ constant assortment.
    Captures ISBNs, pricing, condition grades, and Japanese/English bibliographic
    metadata.

    httpx + BeautifulSoup (server-rendered product pages).
    ...
    BROWSE_CANDIDATES = [
        "/search/?keyword=&condition=used&category=book",
        "/search/?keyword=&category=book",
        "/search/?keyword=",
        "/books",
        "/used-books",
        "/search",
    ...
    DETAIL_SIGNALS = ["/detail/", "/goods/", "/product/", "/book/"]
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="BOOKOFF", territory="Japan")
    ...
    def run(self):
        self.logger.info(
            f"Starting BOOKOFF harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
    ...
    def _harvest_item(self, url: str):
        m = re.search(r"/(?:detail|goods|product|book)/([^/?#]+)", url)
        item_id = re.sub(r"[^a-zA-Z0-9_-]", "_", m.group(1))[:80] if m else re.sub(
            r"[^a-zA-Z0-9_-]", "_",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BOOKOFF Japan cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    BookoffSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
