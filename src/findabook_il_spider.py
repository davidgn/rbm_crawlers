...
class FindabookIlSpider(BaseSpider):
    """
    Findabook.co.il — Israel main-universe used-book marketplace.

    Israeli peer-to-peer used-book buy/sell platform with Hebrew and English
    titles.  Captures ISBNs, pricing, condition, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse paths probed at startup.  Pagination: ?page=N.
    """

    BASE_URL = "https://www.findabook.co.il"
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="Findabook", territory="Israel")
    ...
    def run(self):
        self.logger.info(
            f"Starting Findabook harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Findabook Israel cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    FindabookIlSpider(limit_pages=args.limit).run()
