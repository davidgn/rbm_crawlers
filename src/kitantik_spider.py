...
class KitantikSpider(BaseSpider):
    """
    Kitantik (kitantik.com) — Turkey main-universe used/antique book marketplace.

    Turkish platform for antique, collectible, and used books.  Captures ISBNs,
    pricing, condition, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse: /kitap/.  Pagination: ?page=N.
    """

    BASE_URL = "https://www.kitantik.com"
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Kitantik", territory="Turkey")
    ...
    def run(self):
        self.logger.info(
            f"Starting Kitantik harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
    ...
    def _extract_links(self, soup: BeautifulSoup, seen: set) -> list[str]:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if (
                ...
                and not re.search(r"/kitap/?$", href.rstrip("/"))
            ):
                links.append(href)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kitantik Turkey cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    KitantikSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
