...
class AngolivrosSpider(BaseSpider):
    """
    Angolivros (angolivros.ao) — Angola main-universe bookstore.

    Angolan online bookstore with Portuguese and African-interest titles.
    ...
    DETAIL_SIGNALS = ["/product/", "/produto/", "/livro/", "/book/", "/item/"]
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="Angolivros", territory="Angola")
    ...
    def run(self):
        self.logger.info(
            f"Starting Angolivros harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Angolivros Angola cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    AngolivrosSpider(limit_pages=args.limit).run()
