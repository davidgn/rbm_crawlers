...
class BookshopOrgSearchSpider(BaseSpider):
    """Base search spider for Bookshop.org regional independent bookstore hubs."""
    ...
    def __init__(self, platform_name: str, subdomain: str, territory: str = "United States", price_currency: str = "USD", limit_pages: int = 5, limit_items: int | None = None, **kwargs):
        ...
        if subdomain and subdomain != 'www':
            self.base_url = f"https://{subdomain}.bookshop.org"
        else:
            self.base_url = "https://bookshop.org"
    ...
    def _get_search_url(self, search_term: str, page: int = 1) -> str:
        ...
        return f"{self.base_url}/search?keywords={encoded_term}&page={page}"

    def run(self, search_term: str = "potter") -> list[BookListing]:
        self.logger.info(f"Starting Bookshop.org search crawler for {self.platform_name}. Limit: {self.limit_pages} pages.")
        scraped_items = []
        ...
        for page in range(1, self.limit_pages + 1):
            ...
            self.logger.info(f"Fetching Bookshop.org page {page}: {url}")
            try:
                ...
                items = soup.select('.book-card, .search-result-item, div.product-card, div.grid-item, div[data-product-id]')
                if not items:
                    items = soup.select('div.product, div.item, li.product-item')
                ...
                page_count = 0
                for item in items:
                    title_elem = item.select_one('.book-title, .title, h2, h3, h4, a.title-link')
                    price_elem = item.select_one('.price, .amount, span.price, .price-wrapper')
                ...
                self.logger.error(f"Error scraping Bookshop.org page {page}: {e}")
                break
