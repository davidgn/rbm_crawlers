...
class TirantCOSpider(HTMLSearchSpider):
    """Broad crawler for Tirant lo Blanch (CO) (Colombia)."""
    def __init__(self, search_term: str = "libros", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Tirant lo Blanch (CO)",
            territory="Colombia",
            base_url="https://tirant.com",
            search_path="co/busqueda?q={search_term}",
            currency="COP",
