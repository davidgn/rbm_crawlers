...
class BookSecondStorySpider(BaseSpider):
    """
    Booksecondstory (booksecondstory.com) — Pakistan main-universe secondhand-book marketplace.

    Pass-110 reverified: individual second-hand sellers transact through the
    platform with confirmed buy/sell mechanics.

    httpx + BeautifulSoup first (Pakistani book sites tend to be server-rendered
    or Shopify/WooCommerce).  Falls back gracefully if pages are thin.

    Browse paths probed at startup.  Pagination: WooCommerce /page/N/ tried
    before ?page=N.
    """
    ...
    def run(self):
        self.logger.info(
            f"Starting Booksecondstory harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
