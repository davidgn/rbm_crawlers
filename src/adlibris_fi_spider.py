...
class AdlibrisFiSpider(HTMLSearchSpider):
    """
    Spider for Adlibris (Finland).
    One of the leading online bookstores in the Nordics.
    Uses curl_cffi to bypass TLS fingerprinting.
    """
    def __init__(self, limit_pages: int = 50):
        super().__init__(
            platform_name="Adlibris Finland",
            base_url="https://www.adlibris.com/fi",
            search_path="haku?q={query}&page={page}",
            selectors={
                'container': '.column-box__item', 
                'title': 'h2.title', # Need to verify in smoke
                'link': 'a',
                'price': '.price',
            },
            territory="Finland",
    ...
    def run(self, search_term="Potter"):
        self.logger.info(f"Starting Adlibris (FI) crawler with curl_cffi. Limit: {self.limit_pages} pages.")
...
if __name__ == "__main__":
    spider = AdlibrisFiSpider(limit_pages=1)
    spider.run()
