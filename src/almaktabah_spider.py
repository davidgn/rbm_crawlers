...
class AlmaktabahSpider(BaseSpider):
    """
    ALMAKTABAH.COM (almaktabah.com) — Lebanon main-universe Arabic bookstore.

    Lebanese Arabic-language online bookstore with scholarly and general titles.
    ...
    BASE_URL = "https://www.almaktabah.com"
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="ALMAKTABAH", territory="Lebanon")
    ...
    def run(self):
        self.logger.info(
            f"Starting ALMAKTABAH harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
