...
class DarAlKutubSpider(BaseSpider):
    """
    Dar al-Kutub al-Ilmiyya — Baghdad (almya-group.com) — Iraq main-universe.

    Baghdad-based Arabic scholarly and general book retailer with an online
    catalog.  Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse paths probed at startup.  Pagination: WooCommerce /page/N/ then ?page=N.
    """

    BASE_URL = "https://www.almya-group.com"
    ...
    def run(self):
        self.logger.info(
            f"Starting Dar al-Kutub harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
