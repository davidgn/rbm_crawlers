...
class CamerBookshopSpider(BaseSpider):
    """
    CamerBookshop Online (camerbookshop.online) — Cameroon main-universe bookstore.

    Cameroonian online bookstore with English and French titles.  Captures
    ...
    BASE_URL = "https://www.camerbookshop.online"
    ...
    DETAIL_SIGNALS = ["/product/", "/produit/", "/book/", "/livre/", "/item/"]
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="CamerBookshop", territory="Cameroon")
    ...
    def run(self):
        self.logger.info(
            f"Starting CamerBookshop harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CamerBookshop Online Cameroon cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    CamerBookshopSpider(limit_pages=args.limit).run()
