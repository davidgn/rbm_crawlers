...
class HeritageAuctionsSpider(NodriverSearchSpider):
    """Broad crawler for Heritage Auctions (Australia)."""
    def __init__(self, search_term: str = "books", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Heritage Auctions",
            territory="Australia",
            base_url="https://www.ha.com",
            search_path="c/search-results.zx?Ntt={search_term}",
            currency="AUD",
            limit_pages=limit_pages,
            limit_items=limit_items,
            item_pattern=r'(<div[^>]*class="[^"]*(?:product|item|card)[^"]*"[^>]*>.*?</div>)',
            url_regex=r'href="([^"]+)"',
            price_regex=r'(?:\$|AUD|)[\s]*([\d,]+(?:\.\d{2})?)',
