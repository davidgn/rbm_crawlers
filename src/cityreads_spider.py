...
CONFIG = SimpleNamespace(platform_name="City Reads", territory="Ghana")


class CityReadsSpider(BaseSpider):
    """
    City Reads Books & Stationery (cityreadsbookstore.com) — Ghana main-universe.

    Ghanaian bookstore with a broad new-book and stationery catalog.
    ...
    def __init__(self, limit_pages=100, limit_items=50):
        super().__init__(platform_name="City Reads", territory="Ghana")
    ...
    def run(self):
        self.logger.info(
            f"Starting City Reads harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="City Reads Ghana cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    parser.add_argument("--limit-pages", type=int)
    parser.add_argument("--limit-items", type=int, default=50)
    args = parser.parse_args()
    CityReadsSpider(limit_pages=args.limit_pages or args.limit, limit_items=args.limit_items).run()
