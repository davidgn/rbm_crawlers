...
class SecondHandBooksLkSpider(BaseSpider):
    """
    2ndhandbooks.lk — Sri Lanka main-universe used-books marketplace.

    Operates physical stores alongside an online listing surface.
    URL confirmed: https://2ndhandbooks.lk/

    Playwright + stealth (consistent with other Sri Lanka spiders).
    Browse paths probed at startup; pagination via ?page=N.
    """
    ...
    def run(self):
        self.logger.info(
            f"Starting 2ndhandbooks.lk harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
