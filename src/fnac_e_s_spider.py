...
class FnacESSpider(NodriverSearchSpider):
    """Broad crawler for Fnac (ES) (Spain)."""
    def __init__(self, search_term: str = "books", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Fnac (ES)",
            territory="Spain",
            base_url="www.fnac.es",
            search_path="SearchResult/ResultList.aspx?Search={search_term}",
            currency="EUR",
