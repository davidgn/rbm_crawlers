...
class JiushujieSpider(BaseSpider):
    """
    旧书街 (jiushujie.com) — China mainland main-universe used-book peer trading community.

    Book-specific used-book transfer/trading community; app and web surfaces describe
    buying, selling, transferring, and sharing idle/campus second-hand books.
    ...
    BASE_URL = "https://www.jiushujie.com"
    ...
    DETAIL_SIGNALS = ["/book/", "/item/", "/detail/", "/product/", "/goods/"]
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="旧书街", territory="China (mainland)")
    ...
    def run(self):
        self.logger.info(
            f"Starting 旧书街 harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="旧书街 China cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    JiushujieSpider(limit_pages=args.limit).run()
