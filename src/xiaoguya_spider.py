...
class XiaoguyaSpider(BaseSpider):
    """
    小谷吖 (xiaoguya.com) — China mainland main-universe book-circularity network.

    Operator-managed circular-book network/bookstore: users scan/sell idle books,
    platform refurbishes and resells second-hand books, with offline circulation points
    and a large circular warehouse.  Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup.
    Browse paths probed at startup.  Pagination: ?page=N.
    """

    BASE_URL = "https://www.xiaoguya.com"
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="小谷吖", territory="China (mainland)")
    ...
    def run(self):
        self.logger.info(
            f"Starting 小谷吖 harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="小谷吖 China cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    XiaoguyaSpider(limit_pages=args.limit).run()
