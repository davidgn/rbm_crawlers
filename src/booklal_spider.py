...
class BookLalSpider(BaseSpider):
    def __init__(self, limit_pages=10):
        ...
        self.base_url = "https://www.booklal.com"
        self.limit_pages = limit_pages
        self.client = httpx.Client(timeout=30.0, follow_redirects=True)

    def run(self):
        self.logger.info(f"Starting BookLal crawler. Limit: {self.limit_pages} pages.")
        ...
        self.logger.info(f"Finished BookLal. Scraped {self.items_scraped} items.")

if __name__ == "__main__":
    spider = BookLalSpider(limit_pages=3)
    spider.run()
