...
class KongfzSpider(BaseSpider):
    """
    孔夫子旧书网 (kongfz.com) — China mainland main-universe used/antiquarian book marketplace.

    China's largest used and antiquarian book platform with millions of listings
    from individual and professional sellers.  Captures ISBNs, pricing, condition,
    and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse: /book/.  Pagination: ?page=N.
    """
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="孔夫子旧书网", territory="China (mainland)")
    ...
    def run(self):
        self.logger.info(
            f"Starting 孔夫子旧书网 harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
    ...
    def _extract_links(self, soup: BeautifulSoup, seen: set) -> list[str]:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if (
                ...
                and not re.search(r"/book/?$", href.rstrip("/"))
            ):
                links.append(href)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="孔夫子旧书网 China cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    KongfzSpider(limit_pages=args.limit).run()
