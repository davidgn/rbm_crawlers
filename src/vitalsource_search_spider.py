...
class VitalSourceSearchSpider(BaseSpider):
    ...
    def __init__(self, platform_name: str, subdomain: str, territory: str = "United States", price_currency: str = "USD", limit_pages: int = 5, limit_items: int | None = None, **kwargs):
        ...
        self.base_url = f"https://{subdomain}.vitalsource.com"
    ...
    def _get_search_url(self, search_term: str, page: int = 1) -> str:
        ...
        return f"{self.base_url}/textbooks?term={encoded_term}&page={page}"

    def run(self, search_term: str = "potter") -> list[BookListing]:
        self.logger.info(f"Starting VitalSource search crawler for {self.platform_name}. Limit: {self.limit_pages} pages.")
        scraped_items = []
        ...
        for page in range(1, self.limit_pages + 1):
            ...
            self.logger.info(f"Fetching VitalSource page {page}: {url}")
            try:
                ...
                items = soup.select('.product-card, .search-result-item, .item-card, div[data-vbid], .vbid-card')
                if not items:
                    items = soup.select('div.search-result, div.item, li.result-item')
                ...
                page_count = 0
                for item in items:
                    title_elem = item.select_one('.product-title, .title, h3, h4, a.title-link')
                    price_elem = item.select_one('.product-price, .price, .amount, span.price')
                ...
                self.logger.error(f"Error scraping VitalSource page {page}: {e}")
                break
