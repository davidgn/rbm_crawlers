...
class BeggLireSpider(BaseSpider):
    """
    Bëgg Lire (begglire.com) — Senegal main-universe book marketplace.

    Senegalese book buy/sell/exchange platform with French and local-language
    ...
    BASE_URL = "https://www.begglire.com"
    ...
    def run(self):
        self.logger.info(
            f"Starting Bëgg Lire harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bëgg Lire Senegal cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    BeggLireSpider(limit_pages=args.limit).run()
