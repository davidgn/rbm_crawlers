...
class MyBookOneSpider(BaseSpider):
    """
    一本 MyBookOne (mybookone.com.hk) — Hong Kong main-universe book marketplace.

    Hong Kong buy/sell/exchange book platform with Traditional Chinese and
    English titles.  Captures ISBNs, pricing, condition, and bibliographic
    metadata.
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="一本 MyBookOne", territory="Hong Kong")
    ...
    def run(self):
        self.logger.info(
            f"Starting 一本 MyBookOne harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="一本 MyBookOne Hong Kong cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    MyBookOneSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
