...
class PickaBookSpider(BaseSpider):
    """
    PickaBook Used Books Marketplace (pickabook.co.ke) — Kenya main-universe.

    Kenyan peer-to-peer used-book buy/sell platform.  Captures ISBNs, pricing,
    ...
    BASE_URL = "https://pickabook.co.ke"
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="PickaBook", territory="Kenya")
    ...
    def run(self):
        self.logger.info(
            f"Starting PickaBook harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PickaBook Kenya cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    PickaBookSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
