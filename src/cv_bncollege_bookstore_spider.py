...
class CvBookstoreSpider(PlaywrightSearchSpider):
    """Playwright-based spider for Cv Bookstore (BN College platform)."""

    def __init__(self, limit_pages: int = 5, limit_items: int | None = None, **kwargs):
        super().__init__(
            platform_name="Cv Bookstore",
            base_url="https://cv.bncollege.com",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Cv Bookstore Playwright Spider")
    parser.add_argument("--query", type=str, default="textbook")
    parser.add_argument("--limit-pages", type=int, default=1)
    parser.add_argument("--limit-items", type=int, default=5)
    args, _ = parser.parse_known_args()
    spider = CvBookstoreSpider(limit_pages=args.limit_pages, limit_items=args.limit_items)
