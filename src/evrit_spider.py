...
class EvritSpider(BaseSpider):
    """
    עברית / e-vrit (e-vrit.co.il) — Israel main-universe Hebrew bookstore.

    Israeli online bookstore specialising in Hebrew-language titles.
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="e-vrit", territory="Israel")
    ...
    def run(self):
        self.logger.info(
            f"Starting e-vrit harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="e-vrit Israel cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    EvritSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
