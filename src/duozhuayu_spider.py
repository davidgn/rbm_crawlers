...
class DuozhuayuSpider(BaseSpider):
    """
    多抓鱼 (duozhuayu.com) — China mainland main-universe operator-managed book recommerce.

    Operator-managed recommerce platform for second-hand books; users scan/sell idle books,
    platform inspects, refurbishes, and resells.  Captures ISBNs, pricing, and metadata.

    httpx + BeautifulSoup.
    Browse: /books.  Pagination: ?page=N.
    """
    ...
    DETAIL_SIGNALS = ["/books/", "/product/", "/item/", "/detail/"]
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="多抓鱼", territory="China (mainland)")
    ...
    def run(self):
        self.logger.info(
            f"Starting 多抓鱼 harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="多抓鱼 China cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    DuozhuayuSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
