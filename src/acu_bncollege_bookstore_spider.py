...
class AcuBookstoreSpider(PlaywrightSearchSpider):
    """Playwright-based spider for Acu Bookstore (BN College platform)."""

    def __init__(self, limit_pages: int = 5, limit_items: int | None = None, **kwargs):
        super().__init__(
            platform_name="Acu Bookstore",
            base_url="https://acu.bncollege.com",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Acu Bookstore Playwright Spider")
    parser.add_argument("--query", type=str, default="textbook")
    parser.add_argument("--limit-pages", type=int, default=1)
    parser.add_argument("--limit-items", type=int, default=5)
    args, _ = parser.parse_known_args()
    spider = AcuBookstoreSpider(limit_pages=args.limit_pages, limit_items=args.limit_items)
