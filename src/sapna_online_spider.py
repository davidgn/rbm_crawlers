...
class SapnaOnlineSpider(HTMLSearchSpider):
    """Broad crawler for SapnaOnline (IN) (IA)."""
    def __init__(self, search_term: str = "libros", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="SapnaOnline (IN)",
            territory="IA",
            base_url="https://www.sapnaonline.com",
            search_path="shop/search?q={search_term}",
            currency="USD",
            limit_pages=limit_pages,
            limit_items=limit_items,
            item_pattern=r'(<div[^>]*class="[^"]*(?:product|item|card)[^"]*"[^>]*>.*?</div>)',
            url_regex=r'href="([^"]+)"',
            price_regex=r'(?:\$|USD|)[\s]*([\d,]+(?:\.\d{2})?)',
