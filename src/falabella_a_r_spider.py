...
class FalabellaARSpider(HTMLSearchSpider):
    """Broad crawler for Falabella (AR) (Argentina)."""
    def __init__(self, search_term: str = "libros", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Falabella (AR)",
            territory="Argentina",
            base_url="https://www.falabella.com.ar",
            search_path="falabella-ar/search?Ntt={search_term}",
            currency="ARS",
            limit_pages=limit_pages,
            limit_items=limit_items,
            item_pattern=r'(<div[^>]*class="[^"]*(?:product|item|card)[^"]*"[^>]*>.*?</div>)',
            url_regex=r'href="([^"]+)"',
            price_regex=r'(?:\$|ARS|)[\s]*([\d,]+(?:\.\d{2})?)',
