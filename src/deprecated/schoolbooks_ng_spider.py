...
class SchoolBooksNgSpider(BaseSpider):
    """
    SchoolBOOKS Nigeria (schoolbooks.com.ng) — Nigeria main-universe marketplace.

    Nigerian school and used-books marketplace connecting buyers and sellers of
    textbooks and general books.  Captures ISBNs, pricing, condition, and
    ...
    BASE_URL = "https://schoolbooks.com.ng"
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="SchoolBOOKS Nigeria", territory="Nigeria")
    ...
    def run(self):
        self.logger.info(
            f"Starting SchoolBOOKS Nigeria harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SchoolBOOKS Nigeria cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    SchoolBooksNgSpider(limit_pages=args.limit).run()
