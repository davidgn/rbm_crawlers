...
class ManyoujingSpider(BaseSpider):
    """
    漫游鲸 (manyoujing.net) — China mainland main-universe book-circularity recommerce platform.

    Operator-managed idle/second-hand book circular-reading platform: users scan/quote/sell
    books, receive pickup, buy refurbished second-hand books at low prices.
    ...
    BASE_URL = "https://www.manyoujing.net"
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="漫游鲸", territory="China (mainland)")
    ...
    def run(self):
        self.logger.info(
            f"Starting 漫游鲸 harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="漫游鲸 China cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    ManyoujingSpider(limit_pages=args.limit).run()
