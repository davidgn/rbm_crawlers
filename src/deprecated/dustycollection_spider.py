...
class DustyCollectionSpider(BaseSpider):
    """
    The Dusty Collection (thedustycollection.com) — Seychelles main-universe.

    Seychellois used-book and collectibles shop with an online catalog.
    ...
    BASE_URL = "https://thedustycollection.com"
    ...
    HEADERS = {
        ...
        "Accept-Language": "en-SC,en;q=0.9,fr;q=0.8",
    }

    def __init__(self, limit_pages=100):
        super().__init__(platform_name="The Dusty Collection", territory="Seychelles")
    ...
    def run(self):
        self.logger.info(
            f"Starting The Dusty Collection harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="The Dusty Collection Seychelles cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    DustyCollectionSpider(limit_pages=args.limit).run()
