...
class ObmenBookSpider(BaseSpider):
    """
    ObmenBook (obmenbookkz.orgs.biz) — Kazakhstan main-universe book exchange/sale platform.

    Almaty-based book exchange and sale platform for used books.
    ...
    BASE_URL = "https://obmenbookkz.orgs.biz"
    ...
    DETAIL_SIGNALS = ["/book/", "/item/", "/product/", "/listing/", "/ad/", "/detail/", "/kniga/"]
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="ObmenBook", territory="Kazakhstan")
    ...
    def run(self):
        self.logger.info(
            f"Starting ObmenBook harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ObmenBook Kazakhstan cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    ObmenBookSpider(limit_pages=args.limit).run()
