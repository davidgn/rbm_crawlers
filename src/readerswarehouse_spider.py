...
class ReadersWarehouseSpider(BaseSpider):
    """
    Readers Warehouse (readerswarehouse.co.za) — South Africa main-universe.

    South African discount book retailer with a large online catalog of
    new and remainder books.  Captures ISBNs, pricing, and bibliographic
    metadata.
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Readers Warehouse", territory="South Africa")
    ...
    def run(self):
        self.logger.info(
            f"Starting Readers Warehouse harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Readers Warehouse South Africa cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    ReadersWarehouseSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
