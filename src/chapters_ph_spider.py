...
class ChaptersSpider(BaseSpider):
    def __init__(self, limit_pages=50, limit_items=None):
        ...
        self.base_url = "https://chapters.ph/collections/all-books"

    def run(self):
        self.logger.info(f"Starting Chapters PH Harvest (Cache-First). Limit: {self.limit_pages} pages.")
    ...
    def _harvest_item(self, context, url):
        ...
        try:
            ...
            item = BookListing(
                ...
                title="Cached Item",
                listing_url=url,
                condition="New (Chapters PH Retail)"
            )
            self.save_item(item)
        finally:
            p_page.close()
...
if __name__ == "__main__":
    ...
    spider = ChaptersSpider(limit_pages=args.limit_pages, limit_items=args.limit_items)
    spider.run()
