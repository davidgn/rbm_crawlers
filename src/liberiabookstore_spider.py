...
class LiberiaBookStoreSpider(BaseSpider):
    """
    Liberia Book Store (liberiabookstore.com) — Liberia main-universe bookstore.

    Liberian online bookstore with a broad catalog of new and general-interest
    titles.  Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse paths probed at startup.  Pagination: WooCommerce /page/N/ then ?page=N.
    """

    BASE_URL = "https://liberiabookstore.com"
    ...
    HEADERS = {
        ...
        "Accept-Language": "en-LR,en;q=0.9",
    }

    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Liberia Book Store", territory="Liberia")
    ...
    def run(self):
        self.logger.info(
            f"Starting Liberia Book Store harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Liberia Book Store cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    LiberiaBookStoreSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
