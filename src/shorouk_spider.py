...
class ShoroukSpider(BaseSpider):
    """
    Shorouk Bookstores (shoroukbookstores.com) — Egypt main-universe bookstore.

    Large Egyptian bookselling rail with Arabic and English titles.
    Captures ISBNs, pricing, and bilingual bibliographic metadata.
    ...
    def run(self):
        self.logger.info(
            f"Starting Shorouk harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
