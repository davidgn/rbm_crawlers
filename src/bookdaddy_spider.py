...
class BookDaddySpider(BaseSpider):
    """
    BookDaddy (bookdaddyhk.com) — Hong Kong main-universe used-book marketplace.

    Hong Kong peer-to-peer secondhand book buy/sell platform with both English
    and Chinese titles.  Captures ISBNs, pricing, condition, and bibliographic
    metadata.
    ...
    BASE_URL = "https://www.bookdaddyhk.com"
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="BookDaddy", territory="Hong Kong")
    ...
    def run(self):
        self.logger.info(
            f"Starting BookDaddy harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BookDaddy Hong Kong cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    BookDaddySpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
