...
class BookshelfPhSpider(HTMLSearchSpider):
    """
    Spider for Bookshelf PH (Philippines).
    Uses JSON extraction from the web-pixels-manager-setup script tag.
    """
    def __init__(self, limit_pages: int = 50):
        super().__init__(
            platform_name="Bookshelf PH",
            base_url="https://bookshelf.com.ph",
    ...
    def _parse_item(self, item_soup):
        ...
        try:
            ...
            self.logger.error(f"Failed to parse Bookshelf PH JSON: {e}")

if __name__ == "__main__":
    spider = BookshelfPhSpider(limit_pages=1)
    spider.run()
