...
CONFIG = SimpleNamespace(platform_name="Antoine Online", territory="Lebanon")


class AntoineOnlineSpider(BaseSpider):
    """
    Antoine Online / Librairie Antoine (antoineonline.com) — Lebanon main-universe.

    Lebanon's largest and oldest bookstore chain with French, Arabic, and
    English titles.  Captures ISBNs, pricing, and bibliographic metadata.
    ...
    DETAIL_SIGNALS = ["/product/", "/book/", "/item/", "/books/", "/livres/"]
    ...
    HEADERS = {
        ...
        "Accept-Language": "ar-LB,ar;q=0.9,fr;q=0.8,en;q=0.7",
    }

    def __init__(self, limit_pages=100, limit_items=50):
        super().__init__(platform_name="Antoine Online", territory="Lebanon")
    ...
    def run(self):
        self.logger.info(
            f"Starting Antoine Online harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Antoine Online Lebanon cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    parser.add_argument("--limit-pages", type=int)
    parser.add_argument("--limit-items", type=int, default=50)
    args = parser.parse_args()
    AntoineOnlineSpider(limit_pages=args.limit_pages or args.limit, limit_items=args.limit_items).run()
