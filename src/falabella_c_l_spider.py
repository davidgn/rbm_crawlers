...
class FalabellaCLSpider(HTMLSearchSpider):
    """Broad crawler for Falabella (CL) (Chile)."""
    def __init__(self, search_term: str = "libros", limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(
            platform_name="Falabella (CL)",
            territory="Chile",
            base_url="https://www.falabella.cl",
            search_path="falabella-cl/search?Ntt={search_term}",
            currency="CLP",
            limit_pages=limit_pages,
            limit_items=limit_items,
            item_pattern=r'(<div[^>]*class="[^"]*(?:product|item|card)[^"]*"[^>]*>.*?</div>)',
            url_regex=r'href="([^"]+)"',
            price_regex=r'(?:\$|CLP|)[\s]*([\d,]+(?:\.\d{2})?)',
