...
class VanSchaikSpider(BaseSpider):
    """
    Van Schaik (vanschaik.com) — South Africa main-universe academic bookstore.

    Leading South African academic and professional bookseller with broad
    textbook and scholarly coverage.  Captures ISBNs, pricing, edition data,
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Van Schaik", territory="South Africa")
    ...
    def run(self):
        self.logger.info(
            f"Starting Van Schaik harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Van Schaik South Africa cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    VanSchaikSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
