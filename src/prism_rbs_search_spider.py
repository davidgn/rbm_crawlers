...
class PrismRbsSearchSpider(BaseSpider):
    """Base search spider for PrismRBS (inSite) independent campus bookstore platforms."""
    ...
    def _get_search_url(self, search_term: str, page: int = 1) -> str:
        ...
        return f"{self.base_url}/site_search.aspx?type=site&q={encoded_term}"

    def run(self, search_term: str = "potter") -> list[BookListing]:
        self.logger.info(f"Starting PrismRBS search crawler for {self.platform_name}. Limit: {self.limit_pages} pages.")
        scraped_items = []
        ...
        for page in range(1, self.limit_pages + 1):
            ...
            self.logger.info(f"Fetching PrismRBS page {page}: {url}")
            try:
                ...
                items = soup.select('.search-result, .product_item, tr.searchResultRow, .site-search-item')
                if not items:
                    items = soup.select('div.item, div.product, tr.itemRow')
                ...
                page_count = 0
                for item in items:
                    title_elem = item.select_one('.title, .itemTitle, .productName, a.searchResultTitle, h4')
                    price_elem = item.select_one('.price, .itemPrice, .productPrice, .site-search-price')
                ...
                self.logger.error(f"Error scraping PrismRBS page {page}: {e}")
                break
