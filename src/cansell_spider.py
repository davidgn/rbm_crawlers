...
class CansellSpider(BaseSpider):
    """
    Cansell (cansell.info) — Iran main-universe used book marketplace.

    Iranian classifieds/marketplace platform with book listings.  Captures
    ISBNs, pricing, condition, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered).
    Browse paths probed at startup.  Pagination: ?page=N then /page/N/.
    """

    BASE_URL = "https://cansell.info"
    ...
    DETAIL_SIGNALS = ["/product/", "/item/", "/listing/", "/ad/", "/book/", "/detail/"]
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Cansell", territory="Iran")
    ...
    def run(self):
        self.logger.info(
            f"Starting Cansell harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cansell Iran cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    CansellSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
