"""
OL ISBN backfill for small untargeted platforms.
Targets ~295 records across 10+ platforms not covered by other backfills.
"""
...
TARGET_FILES = [
    ...
    "liberbee_listings.jsonl",
    "antikka_net_listings.jsonl",
    "biblio uk_listings.jsonl",
    "yes24_listings.jsonl",
    "reusebooks_listings.jsonl",
    "biblio_ie_listings.jsonl",
    "biblio_es_listings.jsonl",
    "maremagnum_listings.jsonl",
    "luta_lv_listings.jsonl",
    "bogbasen_dk_listings.jsonl",
    "boekwinkeltjes_listings.jsonl",
    "boekwinkeltjes_be_listings.jsonl",
    "bibliomarket_listings.jsonl",
]
...
NON_LATIN_RE = re.compile(r"[ऀ-ॿ઀-૿ఀ-౿ಀ-೿ഀ-ൿ฀-๿؀-ۿ一-鿿ก-ไ]")
...
def main():
    ...
    for i, (fpath, rec) in enumerate(all_eligible, 1):
        ...
        if not args.dry_run and args.checkpoint_every and i % args.checkpoint_every == 0:
            ckpt_files = {fp for fp, r in all_eligible[:i] if r.get("isbn") or r.get("catalog_ids")}
    ...
    print(f"\nDone. isbn={found_isbn} catalog={found_catalog} no_result={no_result} no_match={no_match}")
