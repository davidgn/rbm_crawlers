...
class ExclusiveBooksSpider(BaseSpider):
    """
    Exclusive Books (exclusivebooks.co.za) — South Africa main-universe bookstore.

    South Africa's leading independent bookstore chain with a broad new-book
    catalog across all genres and a significant online presence.
    Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup (Shopify-based, server-rendered catalog pages).
    Browse paths probed at startup.  Pagination: Shopify ?page=N.
    """
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Exclusive Books", territory="South Africa")
    ...
    def run(self):
        self.logger.info(
            f"Starting Exclusive Books harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Exclusive Books South Africa cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    ExclusiveBooksSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
