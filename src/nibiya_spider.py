...
class NibiyaSpider(BaseSpider):
    """
    Nibiya.com (nibiya.com) — Chad main-universe general classifieds, books category.

    Pan-African classifieds platform with a significant Chadian book-listing
    surface.  Targets the livres/books category.  Captures pricing, condition,
    ...
    DETAIL_SIGNALS = ["/annonce/", "/ad/", "/listing/", "/item/", "/detail/"]
    ...
    HEADERS = {
        ...
        "Accept-Language": "fr-TD,fr;q=0.9,en;q=0.8",
    }

    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Nibiya", territory="Chad")
    ...
    def run(self):
        self.logger.info(
            f"Starting Nibiya harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
    ...
    def _harvest_item(self, url: str):
        ...
        try:
            ...
            price_val = None
            price_elem = soup.select_one(".price bdi, .price .amount, p.price, .ad-price")
            if price_elem:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Nibiya Chad classifieds cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    NibiyaSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
