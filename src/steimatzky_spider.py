...
class SteimatzkySpider(BaseSpider):
    """
    Steimatzky (steimatzky.co.il) — Israel main-universe bookstore chain.

    Israel's largest and oldest bookstore chain with broad Hebrew and English
    catalog coverage.  Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup (server-rendered catalog).
    Browse paths probed at startup.  Pagination: ?page=N.
    """

    BASE_URL = "https://www.steimatzky.co.il"
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Steimatzky", territory="Israel")
    ...
    def run(self):
        self.logger.info(
            f"Starting Steimatzky harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Steimatzky Israel cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    SteimatzkySpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
