...
class CrisolPeSpider(BaseAPISpider):
    """
    Spider for Crisol (Peru).
    Cross-pollinated from the buyback-operators repository.
    Uses the Magento 2 GraphQL API for high-speed scraping.
    """
    def __init__(self, limit_pages: int = 50, limit_items: int | None = None):
        super().__init__(platform_name="Crisol", territory="Peru")
        self.limit_pages = limit_pages
        self.limit_items = limit_items
        self.graphql_url = "https://www.crisol.com.pe/graphql"
        
    async def run_async(self):
        self.logger.info(f"Starting Crisol PE GraphQL Spider (Pages: {self.limit_pages})")
        items_scraped = 0
        ...
        async with httpx.AsyncClient() as client:
            for page in range(1, self.limit_pages + 1):
                ...
                for product in items:
                    ...
                    url = f"https://www.crisol.com.pe/{url_key}.html" if url_key else f"https://www.crisol.com.pe/catalogsearch/result/?q={sku}"
        ...
        self.logger.info(f"Crisol spider completed. Scraped {items_scraped} items.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Crisol PE GraphQL Spider")
