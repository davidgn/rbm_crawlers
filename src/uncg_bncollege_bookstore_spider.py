...
class UncgBookstoreSpider(PlaywrightSearchSpider):
    """Playwright-based spider for Uncg Bookstore (BN College platform)."""

    def __init__(self, limit_pages: int = 5, limit_items: int | None = None, **kwargs):
        super().__init__(
            platform_name="Uncg Bookstore",
            base_url="https://uncg.bncollege.com",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Uncg Bookstore Playwright Spider")
    parser.add_argument("--query", type=str, default="textbook")
    parser.add_argument("--limit-pages", type=int, default=1)
    parser.add_argument("--limit-items", type=int, default=5)
    args, _ = parser.parse_known_args()
    spider = UncgBookstoreSpider(limit_pages=args.limit_pages, limit_items=args.limit_items)
