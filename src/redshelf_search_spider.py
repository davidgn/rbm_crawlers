...
class RedShelfSearchSpider(BaseSpider):
    """Base search spider for RedShelf digital campus bookstore platform portals."""
    ...
    def __init__(self, platform_name: str, subdomain: str, territory: str = "United States", price_currency: str = "USD", limit_pages: int = 5, limit_items: int | None = None, **kwargs):
        ...
        self.base_url = f"https://{subdomain}.redshelf.com"
    ...
    def _get_search_url(self, search_term: str, page: int = 1) -> str:
        ...
        return f"{self.base_url}/search/?terms={encoded_term}&page={page}"

    def run(self, search_term: str = "potter") -> list[BookListing]:
        self.logger.info(f"Starting RedShelf search crawler for {self.platform_name}. Limit: {self.limit_pages} pages.")
        scraped_items = []
        ...
        for page in range(1, self.limit_pages + 1):
            ...
            self.logger.info(f"Fetching RedShelf page {page}: {url}")
            try:
                ...
                items = soup.select('.book-card, .search-result-item, .card, .product-card, div[data-book-id]')
                if not items:
                    items = soup.select('div.search-result, div.item')
                ...
                page_count = 0
                for item in items:
                    title_elem = item.select_one('.book-title, .title, h3, h4, a.title-link')
                    price_elem = item.select_one('.book-price, .price, .amount, span.price')
                ...
                self.logger.error(f"Error scraping RedShelf page {page}: {e}")
                break
