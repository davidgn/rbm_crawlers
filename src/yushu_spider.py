...
class YushuSpider(BaseSpider):
    """
    渔书 (22bat.com) — China mainland main-universe book-circularity recommerce platform.

    Operator/community-mediated circular-reading platform: users scan/quote/sell books,
    receive pickup, and buy refurbished second-hand books.  Physical bookshop/borrowing-point
    network.  Captures ISBNs, pricing, and bibliographic metadata.

    httpx + BeautifulSoup.
    Browse: /pc/cms/index/page_id/55 (books section).  Pagination: ?page=N.
    """

    BASE_URL = "https://www.22bat.com"

    BROWSE_CANDIDATES = [
        "/pc/cms/index/page_id/55",
        "/books/",
        "/shop/",
        "/catalog/",
        "",
    ]
    DETAIL_SIGNALS = ["/book/", "/product/", "/item/", "/detail/", "/goods/"]
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="渔书", territory="China (mainland)")
    ...
    def run(self):
        self.logger.info(
            f"Starting 渔书 harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
    ...
    def _find_browse_url(self) -> str:
        ...
        self.logger.warning("No browse path matched — using books section.")
        return self.BASE_URL + "/pc/cms/index/page_id/55"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="渔书 China cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    YushuSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
