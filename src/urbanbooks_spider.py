...
class UrbanBooksSpider(BaseSpider):
    """
    URBAN BOOKS (urbanbooks1.com) — Lebanon main-universe bookstore.

    Lebanese online bookstore with English, French, and Arabic titles.
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="URBAN BOOKS", territory="Lebanon")
    ...
    def run(self):
        self.logger.info(
            f"Starting URBAN BOOKS harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="URBAN BOOKS Lebanon cache-first spider")
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()
    UrbanBooksSpider(limit_pages=args.limit).run()
