...
class LisMoiSpider(BaseSpider):
    """
    Lis-Moi (lis-moi.com) — Côte d'Ivoire used-book marketplace.

    Ivorian dedicated used-books buy/sell/exchange platform with French titles.
    ...
    def __init__(self, limit_pages: int = 100, limit_items: int | None = None):
        super().__init__(platform_name="Lis-Moi", territory="Côte d'Ivoire")
    ...
    def run(self):
        self.logger.info(
            f"Starting Lis-Moi harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lis-Moi Côte d'Ivoire cache-first spider")
    parser.add_argument("--limit-pages", type=int, default=100)
    parser.add_argument("--limit-items", type=int, default=None)
    args = parser.parse_args()
    LisMoiSpider(limit_pages=args.limit_pages, limit_items=args.limit_items).run()
