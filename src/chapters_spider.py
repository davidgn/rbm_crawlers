...
class ChaptersSpider(BaseSpider):
    """
    Chapters (chapters.ph) — Philippines main-universe pre-loved book marketplace.

    Chapters is a Philippine community marketplace for buying/selling pre-loved
    books. It renders as a modern SPA; Playwright + stealth is used for access.

    Browse paths are probed at startup. Detail URLs are identified by common
    marketplace path patterns. Pagination: ?page=N on the confirmed browse URL.
    """
    ...
    def __init__(self, limit_pages=100):
        super().__init__(platform_name="Chapters", territory="Philippines")
        self.limit_pages = limit_pages
        self.base_url = "https://chapters.ph"

    def run(self):
        self.logger.info(
            f"Starting Chapters harvest (cache-first). limit_pages={self.limit_pages}"
        )
        seen: set[str] = set()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chapters (Philippines) cache-first spider")
    parser.add_argument("--limit", type=int, default=100,
                        help="Max listing-index pages to crawl (default: 100)")
    args = parser.parse_args()
    ChaptersSpider(limit_pages=args.limit).run()
