...
for entry in registry:
    ...
    # Note: manifest IDs often have suffixes like _Person, _Org, etc.
    found_in_manifest = False
    for eid in existing_ids:
        if eid.startswith(source + "_") or eid == source:
            found_in_manifest = True
            break
            
    if implemented and is_aligned and not found_in_manifest:
        candidates.append(entry)

print(f"Found {len(candidates)} candidates.")
for c in candidates[:40]:
    print(f"{c['source']}: {c.get('wd_property', 'N/A')} - {c.get('endpoint_kind', 'N/A')}")
    print(f"  Notes: {c.get('notes', '')[:100]}...")
