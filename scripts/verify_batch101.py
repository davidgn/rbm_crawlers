import requests
import re
import time

targets = [
    {
        "name": "Aboriginal Studies Press",
        "url": "https://aiatsis.gov.au/shop/products/yawulyu",
        "pattern": r"/products/[\w-]+"
    },
    {
        "name": "Australian Law Reform Commission",
        "url": "https://www.alrc.gov.au/publication/safe-informed-supported-report-143/",
        "pattern": r"/publications/[\w-]+"
    },
    {
        "name": "Te Papa Press",
        "url": "https://www.tepapastore.co.nz/products/greta-valdin",
        "pattern": r"/products/[\w-]+"
    },
    {
        "name": "NZ Law Commission",
        "url": "https://www.lawcom.govt.nz/publications",
        "pattern": r"/publications/[\w-]+"
    }
]

print("--- SMOKE TEST: BATCH 101 (Batch 18 & Backlog Repairs) ---")

for t in targets:
    print(f"\nTesting {t['name']}:")
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        resp = requests.get(t['url'], headers=headers, timeout=15, verify=False, allow_redirects=True)
        if resp.status_code == 200:
            print(f"  [SUCCESS] URL resolves (200 OK)")
            if re.search(t['pattern'], resp.url):
                print(f"  [SUCCESS] Pattern '{t['pattern']}' matches URL.")
            else:
                print(f"  [WARNING] Pattern '{t['pattern']}' does NOT match URL: {resp.url}")
        elif resp.status_code == 403:
            print(f"  [FAILED] URL returned status code: 403 (Forbidden / WAF)")
        else:
            print(f"  [FAILED] URL returned status code: {resp.status_code}")
    except Exception as e:
        print(f"  [ERROR] {e}")
    time.sleep(1)
