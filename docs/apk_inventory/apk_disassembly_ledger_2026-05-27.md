...
| `rebuy - Kaufen & Verkaufen_2026.05.5297_APKPure.xapk` | `jadx_disassembled_partial_endpoint_attribution` | Directly relevant to Rebuy buyback endpoint shape/rate-limit research; JADX workspace `decompiled/Rebuy_jadx` attributed the `https://api.rebuy.com` base, emitted green/sell DTOs and last-scanned models, but the `X-API-KEY` value and `customers/green-last-scanned/sync` call-site remain unattributed in default Java output. |
