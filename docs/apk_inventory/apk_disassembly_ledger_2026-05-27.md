...
| `孔夫子旧书网_8.3.1_APKPure.apk` | `raw_string_deep_link_inventory` | China used-book target; unpacked workspace exists at `decompiled/Kongfuzi`. Static pass recovered `kongfz://app.kongfz.com` deep links, add-book routes, `/invokeSearch/app/cate/index`, mobile web detail leads under `m.kongfz.cn`/`m.kongpm.com`, and ISBN/scan UI evidence, but no clean unauthenticated inventory REST API. |
| `闲鱼_7.26.20_APKPure.apk` | `unpacked_mtop_endpoint_inventory` | China horizontal marketplace; unpacked workspace exists at ignored local path `decompiled/Xianyu/unzip`. Static pass recovered Goofish/IdleFish/Taobao hosts, search-result templates, and MTop leads including `mtop.taobao.idle.city.item.search`, `mtop.taobao.idle.barcode.scan`, `mtop.etao.kaka.barcode.search`, and item/detail operations. Live use likely requires signed/authenticated MTop request construction. |
...
| `wer-zahlt-mehr-3-1-0.apk` | `cordova_source_endpoint_attributed` | Buyback comparison target; unpacked workspace exists at `decompiled/Werzahltmehr`. Cordova source confirms `https://www.werzahltmehr.de/mapi/`, API key `f3fc7cf047ccb45239a8642d2391813d`, JSONP quote route `getResults/{apiKey}/{ean}`, and `POST createPackage/` with username/password/API key/EAN-list params. Prior live probe returned 404/Nuxt, so treat the APK route as historically confirmed but likely retired. |
