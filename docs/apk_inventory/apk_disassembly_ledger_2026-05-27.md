...
| `rebuy - Kaufen & Verkaufen_2026.05.5297_APKPure.xapk` | `jadx_disassembled_dex_string_route_inventory` | Directly relevant to Rebuy buyback endpoint shape/rate-limit research; JADX workspace `decompiled/Rebuy_jadx` attributed the `https://api.rebuy.com` base and emitted green/sell DTOs and last-scanned models. A 2026-06-02 raw DEX string pass recovered route leads including `/customers/green-last-scanned`, `/customers/green-last-scanned/statistics`, `/customers/green-last-scanned/sync`, `/customers/my/products`, `/products/bulk`, `/v3/green/carts`, and `/add-all-last-scanned-items`; the `X-API-KEY` value and exact authenticated request schemas remain unattributed. |
