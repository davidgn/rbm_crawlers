...
public class Path implements Iterable<ChildKey>, Comparable<Path> {
    ...
    public boolean contains(Path path) {
        ...
        int i4 = path.start;
        while (i < this.end) {
            if (!this.pieces[i].equals(path.pieces[i4])) {
                return false;
            }
            i++;
            i4++;
    ...
    public boolean equals(Object obj) {
        if (obj instanceof Path) {
            ...
            int i = this.start;
            for (int i4 = path.start; i < this.end && i4 < path.end; i4++) {
                if (!this.pieces[i].equals(path.pieces[i4])) {
                    return false;
                }
                i++;
    ...
    public int hashCode() {
        ...
        for (int i4 = this.start; i4 < this.end; i4++) {
            i = (i * 37) + this.pieces[i4].hashCode();
        }
        return i;
    }
    ...
    public int compareTo(Path path) {
        ...
        while (true) {
            ...
            int compareTo = this.pieces[i4].compareTo(path.pieces[i5]);
            if (compareTo != 0) {
                return compareTo;
            }
            i4++;
    ...
    public Path(String str) {
        ...
        for (String str3 : split) {
            if (str3.length() > 0) {
                this.pieces[i4] = ChildKey.fromString(str3);
                i4++;
            }
        }
        this.start = 0;
    ...
    private Path(ChildKey[] childKeyArr, int i, int i4) {
