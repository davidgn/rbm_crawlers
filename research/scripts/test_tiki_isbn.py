...
def run():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Fetch a list of products in the English books category (320)
            list_res = page.evaluate('''async () => {
                const res = await fetch("https://tiki.vn/api/personalish/v1/blocks/listings?limit=5&categoryId=320");
            ...
            for pid in product_ids:
                ...
                # Check for ISBN in specifications
                isbn = None
                for spec in detail_res.get('specifications', []):
                    for attr in spec.get('attributes', []):
                        if attr.get('code') == 'isbn' or 'isbn' in attr.get('name', '').lower() or attr.get('code') == 'barcode':
                            isbn = attr.get('value')
                
                print(f"Product {pid}: {detail_res.get('name')}")
                print(f"ISBN: {isbn}")
                
                # Print all specification attributes just in case
