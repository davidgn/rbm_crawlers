...
| 8 | More Low-Cost Bookstore Spiders | Implement and upgrade bookstore-specific crawlers for Algeria (Livres & Lis), North Macedonia (Matica Makedonska), Kosovo (House of Books), Tunisia (Dar El Kitab), Bolivia (Encantalibros), and Paraguay (Libruras) using WooCommerce API, and Venezuela (Libronomia) using Shopify suggest API | none | COMPLETED |
